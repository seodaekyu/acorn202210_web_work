...
/*
 * [ 요청을 가로채서 중간에 원하는 작업을 할 수 있는 filter 만들기 ]
 * 
 * 1. javax.servlet.Filter 인터페이스를 구현한다.
 * 2. 어떤 요청에 필터링을 할 것인지 맵핑한다.
 */
@WebFilter("/private/*")
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		//1. 로그인 된 클라이언트인지 확인한다.
		//부모 type을 자식 type으로 casting
		HttpServletRequest req = (HttpServletRequest)request;
		//자식 type을 이용해서 HttpSession 객체의 참조 값을 얻어낸다.
		...
		if(id!=null) {
			...
			//3. 로그인을 하지 않았으면 로그인 폼으로 이동 할 수 있도록 리다이렉트 응답을 준다.
