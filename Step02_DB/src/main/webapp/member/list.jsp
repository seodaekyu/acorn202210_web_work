...
<body>
	<%--
		...
		- 포함된 jsp 페이지에 파라미터를 전달 할 수가 있다.
		- 파라미터를 적절히 전달해서 포함된 jsp 페이지 내에서 해당 파라미터를 추출해서 어떤 로직을 수행 할 수 있다.
		- 아래 같은 경우는 thisPage 라는 파라미터 명으로 member 라는 문자열을 전달하는 것이다.
		  그러면 마치 navbar.jsp?thisPage=member를 요청한 것과 비슷한 효과가 되어
		  navbar.jsp 페이지 내부에서 
		  
		  String thisPage = request.getParameter("thisPage")
		  
		  코드를 수행하면 thisPage 안에는 "member" 라는 문자열이 들어있게된다.
	 --%>
